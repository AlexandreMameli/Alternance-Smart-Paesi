...
class CSV:
    ...
    @staticmethod
    def write(content, mount = conf.MOUNT, file = conf.DATA_FILE + EXTENTION, mode = "a", delimiter = DELIMITER):
        """
    ...
    @staticmethod
    def writeStr(content, mount = conf.MOUNT, file = conf.DATA_FILE + EXTENTION, mode = "a", delimiter = DELIMITER):
        if len(content) is 0:
            return
        # Write data
        with open("/" + mount + "/" + file, mode) as file:
